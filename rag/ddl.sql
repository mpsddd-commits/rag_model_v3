...
-- ╔══════════════════════════════════════════════════════════════════╗
-- ║  Alu-ESG Platform — DB 스키마 v0.4 (통합 최신판)                   ║
-- ║  2026-05-28 · App.jsx (2,629줄) 기준 · 전체 28개 테이블            ║
-- ║                                                                  ║
-- ║  ── 명명 규칙 ──                                                   ║
...
CREATE DATABASE IF NOT EXISTS `esg_scm`
...
CREATE TABLE `ALARM` (
    `id` BIGINT(20) NOT NULL AUTO_INCREMENT COMMENT '알림 ID (PK)',
    `partner_id` BIGINT(20) NOT NULL COMMENT '협력사 ID (FK→COMPANY.partner_id)',
    `company_id` BIGINT(20) NOT NULL COMMENT '관련 기업 ID (FK→COMPANY.id)',
    `type` VARCHAR(30) NOT NULL COMMENT '유형 (RISK/URGENT/INSPECT/SELF/INVITE/AI_AGENT)' COLLATE 'utf8mb4_unicode_ci',
    `level` VARCHAR(10) NULL DEFAULT NULL COMMENT '수준 (fail/warn/info)' COLLATE 'utf8mb4_unicode_ci',
    `title` VARCHAR(200) NOT NULL COMMENT '제목' COLLATE 'utf8mb4_unicode_ci',
    `content` TEXT NULL DEFAULT NULL COMMENT '본문' COLLATE 'utf8mb4_unicode_ci',
    `path` VARCHAR(300) NULL DEFAULT NULL COMMENT '클릭 시 이동 경로' COLLATE 'utf8mb4_unicode_ci',
    `meta_json` LONGTEXT NULL DEFAULT NULL COMMENT '메타 데이터' COLLATE 'utf8mb4_bin',
    `is_read` TINYINT(1) NOT NULL DEFAULT '0' COMMENT '읽음 여부 (0=안읽음)',
    `delete_yn` TINYINT(1) NOT NULL DEFAULT '0' COMMENT '삭제 여부',
    `created_at` DATETIME NOT NULL DEFAULT current_timestamp() COMMENT '생성일시',
    PRIMARY KEY (`id`) USING BTREE,
    INDEX `idx_user_read` (`partner_id`, `is_read`, `delete_yn`) USING BTREE,
    CONSTRAINT `meta_json` CHECK (json_valid(`meta_json`))
)
COMMENT='알림 — NotificationPanel / notify.py 호환'
...
-- 10.10 ALARM
INSERT INTO `ALARM` (user_id,company_id,type,level,title,content,is_read,created_at) VALUES
(1,1,'RISK','fail','FEOC 초과 — (주)케이알엠','FEOC 원료 12.5% IRA 위험.',0,'2026-05-19 09:15:00'),
(1,1,'URGENT','warn','긴급 요청 — RM-003','전해망간 원산지 증명서 제출.',0,'2026-05-18 16:30:00'),
(1,1,'SELF','info','자가진단 제출 — Comilog','SAR-2025-0001 제출.',1,'2026-05-17 14:05:00');
...
-- ╔══════════════════════════════════════════════════════════════════╗
-- ║  📊 DB v0.4 통계 — 총 28개 테이블 (UPPER_CASE 통일)                ║
-- ║  ─────────────────────────────────────────────                    ║
-- ║  S1. 사용자·권한·메뉴·알림     : 5개                               ║
-- ║  S2. 기업·초대                 : 2개                               ║
-- ║  S3. PO                        : 1개                               ║
