...
def export_pgvector_to_file_and_hf(repo_id="Makesols/esg-vector-dataset3"):
    ...
    if not rows:
        print("[경고] DB에 저장된 데이터가 없어 백업 및 업로드를 건너뜁니다.")
        return
    ...
    print("[프로세스] 데이터셋 유니크 해시 키 및 타임스탬프 생성 중...")
    ...
    print("[프로세스] ESG/알루미늄 성분 및 수치 규격(%, Mpa) 태그 추출 중...")
    ...
    print("[성공] 로컬 멀티-포맷 백업 완료 (esg_vector_backup.parquet)")

    # 5. [추천 로직 C] Hugging Face Hub 구조화 업로드
    if repo_id:
        try:
            print(f"[HF] 허깅페이스 프라이빗 리포지토리 '{repo_id}' 업로드 준비 중...")
            ...
            print(f"🎉 [최종 성공] 고도화된 메타데이터를 포함한 HF 데이터셋 업로드가 전면 완료되었습니다!")
            
        except Exception as e:
            print(f"[오류] 허깅페이스 업로드 중 실패 (토큰 인증 또는 네트워크 바인딩 오류): {e}")
