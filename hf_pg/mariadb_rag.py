...
# 3. [기능 2] TextArea 텍스트에서 AI로 수치 규격을 추출한 후, 파이썬으로 안전하게 정밀 비교
def extract_and_compare(user_query, target_text):
    """
...
# =======================================================================
# 4. [기능 추가] 뼈대들을 조립하여 최종 답변을 생성하고 로그를 남기는 마스터 함수
# =======================================================================
def process_esg_query(user_query):
    """
    ...
    print(f"[시스템] 매칭된 지표 발견 -> 지표번호 {matched_row['indicator_no']}: {matched_row['indicator_name']}")
    
    # ── Step 2. AI 수치 파싱 및 파이썬 정밀 비교 가동
    # matched_row['question'] (TextArea)을 던져서 기준 수치를 뜯어내고 비교합니다.
    status, base_val = extract_and_compare(user_query, matched_row['question'])
    ...
    print(f"[시스템] 판정 완료 -> 결과: [{status}] (기준치: {base_val})")
    ...
    try:
        ...
        print("[시스템] DB에 AI 판정 로그 저장이 완료되었습니다.")
    except Exception as log_error:
        print(f"[경고] 로그 저장 중 오류 발생 (시스템 작동에는 문제 없음): {log_error}")

    return final_answer


# =======================================================================
# 5. [테스트 블록] 단독 실행 시 로컬에서 즉시 테스트할 수 있는 진입점
# =======================================================================
if __name__ == "__main__":
    # ※ 주의: 이 테스트를 돌리기 전 MariaDB에 esg_checklist 테이블과 데이터가 있어야 합니다.
    print("====== ESG MariaDB RAG 시스템 독립 실행 테스트 ======")
